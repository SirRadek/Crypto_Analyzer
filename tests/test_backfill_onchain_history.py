...
def test_backfill_onchain_history(tmp_path, monkeypatch):
    ...
    df = pd.read_sql_query("SELECT * FROM onchain_1d ORDER BY ts_utc", conn)
    conn.close()

    assert list(df["ts_utc"]) == [1609459200, 1609545600]
...
def test_ensure_schema_adds_missing_columns(tmp_path):
    ...
    with sqlite3.connect(db_path) as conn2:
        ...
        columns = {row[1] for row in conn2.execute("PRAGMA table_info(onchain_1d)")}
        indexes = {row[1] for row in conn2.execute("PRAGMA index_list(onchain_1d)")}
